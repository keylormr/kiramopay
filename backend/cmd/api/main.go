...
func main() {
	...
	healthHandler := func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, `{"status":%q,"version":"1.0.0","environment":%q,"services":{"database":%q,"redis":%q},"websocket_clients":%d,"last_drift_crc":%d,"crypto_prices":%s}`,
			status, cfg.Server.Environment, dbOk, redisOk, wsHub.ClientCount(), reconcileSvc.LastDriftCRC(), cripto)
	}
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Delete("/qr/merchants/{id}/staff/{staffID}", qrHandler.RevokeStaff)
			r.Get("/qr/merchants/{id}/locations", qrHandler.ListLocations)
			r.Post("/qr/merchants/{id}/locations", qrHandler.CreateLocation)
			r.Put("/qr/merchants/{id}/locations/{locationID}", qrHandler.UpdateLocation)
			r.Get("/qr/merchants/{id}/catalog", qrHandler.ListCatalog)
